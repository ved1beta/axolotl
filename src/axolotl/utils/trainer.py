...
def calculate_total_num_steps(cfg, train_dataset, update=True):
    if (
        ...
        # Avoid .to_pandas() on the input_ids list column — it materializes
        # every token as a Python int (~1.3TB on a 9.5M-row long-context
        # dataset; see issue #2975). Use the precomputed length column when
        # available, otherwise compute list lengths via pyarrow.
